# Tempo — Product Requirements (v1, conference-talk framing — SUPERSEDED)

> **Superseded on 2026-06-17 by the root `PRD.md`.** This is the original talk-first
> requirements doc, preserved for history. The project has been re-baselined to lead with
> *model fidelity* — semantically-named validity periods and a domain **operations layer**
> that captures how facts accumulate and change over time — with the demo following rather
> than driving (see `DECISIONS.md` ADR-017). The v1→v2 migration described below is retained
> as a historical artifact, no longer the sole centerpiece (ADR-024). Read `PRD.md` for the
