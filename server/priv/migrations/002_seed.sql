...
WITH e AS (
  INSERT INTO event_log (occurred_at, actor, operation, summary, payload) VALUES
    ('2024-01-01', 'seed', 'revise_rate_card', 'Establish rate card (L3-L6) from 2024-01-01',
     '{"levels":{"3":800,"4":1000,"5":1200,"6":1800},"effective":"2024-01-01"}')
  RETURNING id)
...
SELECT v.level, v.day_rate, v.eff, e.id FROM e,
  (VALUES (3, 800.00, daterange('2024-01-01','2027-01-01')),
          ...
          (6, 1800.00, daterange('2024-01-01','2027-01-01'))) AS v(level, day_rate, eff);

-- Set salaries (what we PAY) per level. ---------------------------------------
WITH e AS (
  INSERT INTO event_log (occurred_at, actor, operation, summary, payload) VALUES
    ('2024-01-01', 'seed', 'set_salary', 'Set salaries (L3-L6) from 2024-01-01',
     '{"levels":{"3":6000,"4":8000,"5":10000,"6":14000},"effective":"2024-01-01"}')
  RETURNING id)
