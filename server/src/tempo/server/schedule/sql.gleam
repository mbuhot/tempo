...
/// A row you get from running the `schedule_candidates` query
/// defined in `./src/tempo/server/schedule/sql/schedule_candidates.sql`.
///
/// > 🐿️ This type definition was generated automatically using v4.7.0 of the
/// > [squirrel package](https://github.com/giacomocavalieri/squirrel).
///
pub type ScheduleCandidatesRow {
  ScheduleCandidatesRow(
    engineer_id: Int,
    name: String,
    level: Int,
    proficiency: Float,
...
pub fn schedule_candidates(
  ...
  arg_4: Date,
  arg_5: Date,
) -> Result(pog.Returned(ScheduleCandidatesRow), pog.QueryError) {
  let decoder = {
    use engineer_id <- decode.field(0, decode.int)
    use name <- decode.field(1, decode.string)
    use level <- decode.field(2, decode.int)
    use proficiency <- decode.field(3, pog.numeric_decoder())
    use free <- decode.field(4, pog.numeric_decoder())
    use commitments <- decode.field(5, decode.string)
    decode.success(ScheduleCandidatesRow(
      engineer_id:,
      name:,
      level:,
      proficiency:,
