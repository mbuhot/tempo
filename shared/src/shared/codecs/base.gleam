//// Wire primitives shared by the per-handler command codecs and the top-level
//// read-model codecs in `shared/codecs`: ISO-8601 date encode/parse and the
//// lenient float decoder. It lives BELOW `codecs` in the import graph so the
//// per-command codec modules (`codecs/engineer`, `codecs/allocation`, …) can depend
//// on it without forming a cycle through `codecs`. `codecs` re-exports the public
//// ones so existing `codecs.encode_date`/`codecs.date_decoder`/`codecs.parse_iso_date`
