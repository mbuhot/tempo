//// JSON codec for `ProjectRequirementCommand` — the project capacity-requirement
//// aggregate's slice of the command wire contract (a project's demand for FTE at a
//// level over a bounded window). `encode` tags the variant by its `op`; `decoder`
...
import shared/types.{type ProjectRequirementCommand, SetProjectRequirement}
